...
public class InvisibleFragment extends Fragment {
    ...
    private void onRequestNormalPermissionsResult(@NonNull String[] permissions, @NonNull int[] grantResults) {
        if (checkForGC() && permissions != null && grantResults != null && permissions.length == grantResults.length) {
            ...
            for (String permission : deniedPermissions) {
                if (MoorPermissionX.isGranted(getContext(), permission)) {
                    pb.deniedPermissions.remove(permission);
                    pb.grantedPermissions.add(permission);
                }
            }
    ...
    private void onRequestBackgroundLocationPermissionResult() {
        if (checkForGC()) {
            if (MoorPermissionX.isGranted(getContext(), ACCESS_BACKGROUND_LOCATION)) {
    ...
    private boolean checkForGC() {
        if (pb == null || task == null) {
            MoorLogUtils.w("PermissionX", "PermissionBuilder and ChainTask should not be null at this time, so we can do nothing in this case.");
            return false;
        }
        return true;
    }
