 ...
 * </pre>
 */
public class MoorPagerGridSnapHelper extends SnapHelper {
...
//        Loge("findTargetSnapPosition, pos = " + pos);
        ...
        if (layoutManager instanceof MoorPagerGridLayoutManager) {
            MoorPagerGridLayoutManager manager = (MoorPagerGridLayoutManager) layoutManager;
...
//        Loge("findTargetSnapPosition, velocityX = " + velocityX + ", velocityY" + velocityY);
        if (null != layoutManager && layoutManager instanceof MoorPagerGridLayoutManager) {
            MoorPagerGridLayoutManager manager = (MoorPagerGridLayoutManager) layoutManager;
            if (manager.canScrollHorizontally()) {
                if (velocityX > MoorPagerConfig.getFlingThreshold()) {
                    target = manager.findNextPageFirstPos();
                } else if (velocityX < -MoorPagerConfig.getFlingThreshold()) {
                    target = manager.findPrePageFirstPos();
                }
            } else if (manager.canScrollVertically()) {
                if (velocityY > MoorPagerConfig.getFlingThreshold()) {
                    target = manager.findNextPageFirstPos();
                } else if (velocityY < -MoorPagerConfig.getFlingThreshold()) {
                    target = manager.findPrePageFirstPos();
                }
            }
        }
//        Loge("findTargetSnapPosition, target = " + target);
    ...
    public boolean onFling(int velocityX, int velocityY) {
        ...
        int minFlingVelocity = MoorPagerConfig.getFlingThreshold();
//        Loge("minFlingVelocity = " + minFlingVelocity);
    ...
    protected LinearSmoothScroller createSnapScroller(RecyclerView.LayoutManager layoutManager) {
        ...
        return new MoorPagerGridSmoothScroller(mRecyclerView);
    }
    ...
    public void setFlingThreshold(int threshold) {
        MoorPagerConfig.setFlingThreshold(threshold);
    }
}