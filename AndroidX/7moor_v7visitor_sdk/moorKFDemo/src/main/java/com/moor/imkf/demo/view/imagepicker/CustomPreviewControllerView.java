...
public class CustomPreviewControllerView extends PreviewControllerView {
    ...
    protected int getLayoutId() {
        return R.layout.moor_picture_custom_preview;
    }
...
//        PStatusBarUtil.fullScreenWithCheckNotch((Activity) getContext(), Color.BLACK);
        PStatusBarUtil.setStatusBar((Activity) getContext(),context.getResources().getColor(R.color.moor_color_f5f5f5),false,true);
    }
    ...
    public void onPageSelected(int position, ImageItem imageItem, int totalPreviewCount) {
        ...
        notifyPreviewList();
        refreshNextBtn();
        mTvTitle.setText(position + 1 + "/" + totalPreviewCount);
    }
    ...
    private void refreshNextBtn() {
        ...
        if (index >= 0) {
            ...
            Drawable drawable = PCornerUtils.cornerDrawableAndStroke(context.getResources().getColor(R.color.moor_color_0081ff), dp(30), dp(1), Color.WHITE);
        ...
        if (selectedList == null || selectedList.size() == 0) {
            mTvNext.setText(context.getString(R.string.moor_chat_send_picture));
            mTvNext.setTextColor(context.getResources().getColor(R.color.moor_color_151515));
        } else {
            String text = String.format("%s(%d/%d)", context.getString(R.string.moor_chat_send_picture), selectedList.size(), selectConfig.getMaxCount());
            mTvNext.setText(text);
            mTvNext.setTextColor(context.getResources().getColor(R.color.moor_color_0081ff));
        }
    }
}